...
vector<vector<string> > DiffSession::getDiff(vector<string> linesFile1, vector<string> linesFile2)
{
    ...
    for (int d = 0; d <= totalLinesCount; d++) {
        for (int k = -d; k <= d; k += 2) {
            ...
            while ((x < linesCount1) && (y < linesCount2) && (strCompare(linesFile1[x], linesFile2[y], this->ignoreWhiteSpaces))) {
                x++;
                y++;
                updateList.push_back({ "Keep",
                    linesFile1[x - 1] });
            }
